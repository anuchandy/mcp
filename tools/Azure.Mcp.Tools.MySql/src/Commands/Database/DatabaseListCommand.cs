...
public sealed class DatabaseListCommand(ILogger<DatabaseListCommand> logger) : BaseServerCommand<DatabaseListOptions>(logger)
{
    ...
    public override async Task<CommandResponse> ExecuteAsync(CommandContext context, ParseResult parseResult)
    {
        ...
        try
        {
            ...
            List<string> databases = await mysqlService.ListDatabasesAsync(context.UserContext, options.Subscription!, options.ResourceGroup!, options.User!, options.Server!);
