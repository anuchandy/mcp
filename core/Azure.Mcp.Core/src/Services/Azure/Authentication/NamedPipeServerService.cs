...
public sealed class NamedPipeServerService : IDisposable
{
    ...
    public NamedPipeServerService(
        IOboParentBrokerService brokerService,
        ...
        ILogger<NamedPipeServerService> logger)
    {
        ...
        {
            throw new InvalidOperationException("NamedPipeServerService should only be constructed in OBO parent mode.");
        }

        if (string.IsNullOrWhiteSpace(serviceStartOptions.OboChannel))
        {
            throw new InvalidOperationException("OBO parent must have a non-empty OboChannel to host the broker pipe.");
        }

        _pipeName = serviceStartOptions.OboChannel;
    }
