...
public class ServerParamGetCommandTests
{
    ...
    public async Task ExecuteAsync_ReturnsParameter_WhenSuccessful()
    {
        ...
        _mysqlService.GetServerParameterAsync(McpUserContext.Empty, "sub123", "rg1", "user1", "test-server", "max_connections").Returns(expectedValue);
    ...
    {
        _mysqlService.GetServerParameterAsync(McpUserContext.Empty, "sub123", "rg1", "user1", "test-server", "invalid_param")
