...
public sealed class TableSchemaGetCommand(ILogger<TableSchemaGetCommand> logger) : BaseDatabaseCommand<TableSchemaGetOptions>(logger)
{
    ...
    public override async Task<CommandResponse> ExecuteAsync(CommandContext context, ParseResult parseResult)
    {
        ...
        try
        {
            ...
            List<string> schema = await pgService.GetTableSchemaAsync(context.UserContext, options.Subscription!, options.ResourceGroup!, options.User!, options.Server!, options.Database!, options.Table!);
